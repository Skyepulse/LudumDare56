...
func _ready() -> void:
	...
	top_right_position_2 = (position_camera + (0.5*size_camera)*Vector2(1,-1) + Vector2(0,398))
...
func _process(delta: float) -> void:
	...
	if nbr_route == 2:
		...
		if time_passed >= delay:
			if rand_gauche <= proba_gauche: #probability that there is a car on delay
				voiture_move(top_left_position, true)
			if rand_droite_1 <= proba_droite: #probability that there is a car on delay
				voiture_move(top_right_position, false)
			if rand_droite_2 <= proba_droite: #probability that there is a car on delay
